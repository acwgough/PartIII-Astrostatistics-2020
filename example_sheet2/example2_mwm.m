...
xlabel('log Importance Weights','FontSize',fs)
ylabel('Number','FontSize',fs);
title('Distribution of Importance Weights','FontSize',fs)
text(-500,1200,['Prior samples N = ' num2str(N,'%.0f')],'FontSize',fs)
text(-500,1000,['Effective SS = ' num2str(ess,'%.2f')],'FontSize',fs)
...
xlim([10.5,14.5])
hold off
xlabel('log_{10}[ M_{vir} / M_{solar} ]','FontSize',fs)
ylabel('Probability Density','FontSize',fs)

text(11.2,0.75,'Prior','FontSize',fs,'Color','b')
text(11.5,2,'Posterior','FontSize',fs) 
title('Bayesian Inference of MW Mass','FontSize',fs)
