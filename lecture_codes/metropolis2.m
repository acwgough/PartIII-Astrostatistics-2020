...
figure(1)
subplot(2,1,1)
plot(pars_mc(:,1),'LineWidth',1)
ylabel('\mu')
title(['Markov Chain Trace Plot: N_{mc} = ' num2str(n_mc,'%.0f')])

subplot(2,1,2)
plot(pars_mc(:,2),'LineWidth',1)
xlabel('Chain step')
ylabel('\sigma')
...
figure(2)
set(gca,'FontSize',fs)
plot(pars_mc(:,1),pars_mc(:,2),'LineWidth',1)
...
ylabel('Posterior P(\sigma | y)','FontSize',fs)
hold on
...
xlabel('\mu','FontSize',18)
ylabel('\sigma^2','FontSize',fs)
title('Posterior P(\mu, \sigma | y)','FontSize',fs)
hold on
